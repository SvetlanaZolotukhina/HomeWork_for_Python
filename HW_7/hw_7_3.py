...
class Cell:
    ...
    def __str__(self):
        return f'результат операции {self.cage * "*"}'

    def __add__(self, other):
        return Cell(self.cage + other.cage)

    def __sub__(self, other):
        return Cell(self.cage - other.cage) if (self.cage - other.cage) < 0 else print('Отрицательно')
    ...
    def make_order(self, cells_in_row):
        row = ''
        for i in range(int(self.cage / cells_in_row)):
            row += f'{"*" * cells_in_row} \n' #row += f'{"*" * cells_in_row} \\n'
        row += f'{"*" * (self.cage % cells_in_row)}'
        return row

cell_1 = Cell(44)
cell_2 = Cell(10)
print('Ячейки первой клетки: ', cell_1)
print('Ячейки второй клетки: ', cell_2)
print('Сумма ячеек клеток', cell_1 + cell_2)
print('Разность ячеек клеток', cell_2 - cell_1)
print('По рядам первая клетка')
print(cell_1.make_order(10))
print('По рядам вторая клетка')
print(cell_2.make_order(3))
print('Частное ячеек клеток', cell_1 / cell_2)
